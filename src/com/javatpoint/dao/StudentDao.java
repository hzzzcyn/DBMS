...
public static List<StudentBean> getRecordByRollno(String name, String sex){
	List<StudentBean> list=new ArrayList<StudentBean>();
	PreparedStatement ps=null;
	try{
		Connection con=DB.getCon();
		if(name.equals("null")) {
		ps=con.prepareStatement("select * from fee_student where sex=?");
		ps.setString(1,sex);
		}
		if(sex.equals("null")) {
			ps=con.prepareStatement("select * from fee_student where name=?");
			ps.setString(1,name);
		}
		if(!name.equals("null")&&!sex.equals("null")){
		ps=con.prepareStatement("select * from fee_student where name=? and sex=?");
		ps.setString(1,name);
		ps.setString(2,sex);
		}
		ResultSet rs=ps.executeQuery();
